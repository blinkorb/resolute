...
const buildStatic = async (watch?: boolean) => {
  ...
  const esbuildResult = await esbuild.build({
    entryPoints: [resoluteClientPathname, ...clientFiles],
    outdir: path.resolve(BUILD_PATHNAME, 'bundled'),
    format: 'esm',
    target: 'esnext',
    sourcemap: 'inline',
  ...
  esbuildResult.outputFiles.forEach((file) => {
    mkdirpSync(path.dirname(file.path));
    fs.writeFileSync(file.path, file.contents, { encoding: 'utf8' });
  });

  await cpy(
    path.resolve(BUILD_PATHNAME, 'bundled', `**/*${GLOB_JS_EXTENSION}`),
    STATIC_PATHNAME
  );

  const relativeBundlePaths = esbuildResult.outputFiles.map((file) =>
    path.relative(path.resolve(BUILD_PATHNAME, 'bundled'), file.path)
  );
  ...
  const bundledFiles = glob.sync(
    path.resolve(STATIC_PATHNAME, `**/*${GLOB_JS_EXTENSION}`)
  );

  bundledFiles.forEach((pathname) => {
