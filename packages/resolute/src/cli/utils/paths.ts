...
import { BUILD_DIR, SRC_PATHNAME } from '../constants.js';

export const fromCompiledPathToRelativeTSX = (pathname: string) => {
  const resolved = path.resolve(
    SRC_PATHNAME,
    path
      .relative(path.resolve(BUILD_DIR, 'compiled'), pathname)
      .replace(/\.js$/, '.tsx')
  );
